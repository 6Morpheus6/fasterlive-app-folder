...
class FasterLivePortraitPipeline:
    ...
    def _run(self, src_info, x_d_i_info, x_d_0_info, R_d_i, R_d_0, realtime, input_eye_ratio, input_lip_ratio,
        ...
        for j in range(len(src_info)):
            ...
            if self.cfg.infer_params.flag_relative_motion:
                if self.cfg.infer_params.animation_region in ["all", "pose"]:
                    if self.is_source_video or getattr(self, 'R_d_smooth', None) is not None:
                        R_new = self.R_d_smooth.process(R_d_i)
                        if not self.is_source_video:
                            R_new = (R_new @ np.transpose(R_d_0, (0, 2, 1))) @ R_s
                    else:
                ...
                if self.is_source_video or getattr(self, 'exp_smooth', None) is not None:
                ...
                if self.cfg.infer_params.animation_region in ["all", "exp"]:
                    if self.is_source_video:
                        ...
                        delta_new = x_s_info['exp'] + (x_d_exp_smooth - x_d_0_info['exp'])
                elif self.cfg.infer_params.animation_region in ["lip"]:
                    for lip_idx in [6, 12, 14, 17, 19, 20]:
                        if self.is_source_video:
                            delta_new[:, lip_idx, :] = x_d_exp_smooth[:, lip_idx, :]
                        else:
                            delta_new[:, lip_idx, :] = (x_s_info['exp'] + (x_d_exp_smooth - x_d_0_info['exp']))[:,
                                                       lip_idx, :]
                    ...
                    for eyes_idx in [11, 13, 15, 16, 18]:
                        if self.is_source_video:
                            ...
                            delta_new[:, eyes_idx, :] = (x_s_info['exp'] + (x_d_exp_smooth - x_d_0_info['exp']))[:,
                                                         eyes_idx, :]
            ...
            t_spade_start = time.time()
            out_crop = self.model_dict["warping_spade"].predict(f_s, x_s, x_d_i_new)
            print(f"      [Profile] warping_spade: {(time.time() - t_spade_start)*1000.0:.1f}ms")
        ...
        return np.ascontiguousarray(out_crop.to(dtype=torch.uint8).cpu().numpy()), np.ascontiguousarray(I_p_pstbk.to(dtype=torch.uint8).cpu().numpy())

    def run(self, image, img_src, src_info, **kwargs):
        t_start = time.time()
        img_bgr = image
        img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
        if getattr(self, 'I_p_pstbk_cache', None) is None or getattr(self, 'last_img_src_id', None) != id(img_src):
            self.I_p_pstbk_cache = torch.from_numpy(img_src).to(self.device).float()
            self.last_img_src_id = id(img_src)
        I_p_pstbk = self.I_p_pstbk_cache.clone()
        realtime = kwargs.get("realtime", False)
        
        if kwargs.get("first_frame", False):
            self.driving_global_bbox = None
            self.src_lmk_pre = None
            
        if getattr(self, 'last_img_shape', None) != img_rgb.shape:
            self.driving_global_bbox = None
            self.src_lmk_pre = None
        self.last_img_shape = img_rgb.shape
            
        t0 = time.time()
        if self.cfg.infer_params.flag_crop_driving_video:
            # ALWAYS use robust face analysis to prevent flickering and tracking drift
            ...
            self.driving_global_bbox = global_bbox
            ...
            if getattr(self, 'driving_global_bbox', None) is None:
                ret_bbox = parse_bbox_from_landmark(
                    lmk,
                    scale=self.cfg.crop_params.dri_scale,
                    vx_ratio_crop_video=self.cfg.crop_params.dri_vx_ratio,
            ...
            img_crop = ret_dct["img_crop"]
            img_crop = cv2.resize(img_crop, (256, 256))
        print(f"    [Profile] face analysis/landmark: {(time.time() - t0)*1000.0:.1f}ms")
        ...
        if kwargs.get("first_frame", False) or self.R_d_0 is None:
            ...
            self.R_d_smooth = utils.OneEuroFilter(4, 0.3)
            self.exp_smooth = utils.OneEuroFilter(4, 0.3)
            self.scale_smooth = utils.OneEuroFilter(4, 0.05) # Low cutoff (0.05) to filter out scale pulsing
            self.t_smooth = utils.OneEuroFilter(4, 0.1) # Smooth translation to prevent jitter
            
        if not kwargs.get("first_frame", False) and getattr(self, 'scale_smooth', None) is not None:
            x_d_i_info['scale'] = self.scale_smooth.process(x_d_i_info['scale'])
            x_d_i_info['t'] = self.t_smooth.process(x_d_i_info['t'])
            
        R_d_0 = self.R_d_0.copy()
        x_d_0_info = copy.deepcopy(self.x_d_0_info)
        print(f"    [Profile] motion extractor: {(time.time() - t1)*1000.0:.1f}ms")

        t2 = time.time()
        out_crop, I_p_pstbk = self._run(src_info, x_d_i_info, x_d_0_info, R_d_i, R_d_0, realtime, input_eye_ratio,
                                        input_lip_ratio,
                                        I_p_pstbk, **kwargs)
        print(f"    [Profile] _run pipeline: {(time.time() - t2)*1000.0:.1f}ms")
